...
export default function ServiceLandingHero({ eyebrow, title, description, service, buttonLabel, image, imageAlt, features }) {
  return <section className="skin-banner skin-showcase service-showcase"><div className="container-x skin-banner-inner skin-banner-grid"><div className="skin-banner-content"><span className="skin-showcase-eyebrow">{eyebrow}</span><h1>{title}</h1><p className="skin-banner-description">{description}</p><div className="skin-banner-contact"><Link to={`/contact?service=${service}`}>{buttonLabel} <span aria-hidden="true">→</span></Link><a href="tel:+916239557417"><span aria-hidden="true">☎</span> 062395 57417</a></div><div className="skin-showcase-features">{features.map((feature) => <div key={feature.label}><span><HeroIcon type={feature.icon} /></span><p>{feature.label}</p></div>)}</div></div><div className="skin-showcase-visual service-showcase-visual"><img src={image} alt={imageAlt} /></div></div></section>;
}
