...
export default function Home() {
  return <Layout>
    ...
    <section className="home-hero"><div className="container-x home-hero-grid">
      <div className="hero-copy"><span className="eyebrow">Diet · Skin · Hair · Wellness</span><h1>Diet and Skin Care Clinic <span>in Mohali</span></h1><p className="hero-script">Where Nutrition Meets Aesthetics</p><p className="hero-lead"><strong>Your skin and hair reflect the choices your body is making every day.</strong></p><p>Ouransh combines personalised nutrition with advanced aesthetic treatments. Because acne, pigmentation and hair fall usually begin somewhere deeper than the surface.</p>
    ...
    <section className="services-section"><div className="container-x"><div className="section-heading"><span className="eyebrow">Care designed around you</span><h2>What We Offer</h2><p>Clinical expertise and personalised nutrition, brought together under one roof.</p></div><div className="service-grid">{services.map((s,i)=><article className="service-card" key={s.title}><div className="service-image"><img src={s.img} alt={s.title}/><span>0{i+1}</span></div><div className="service-body"><div className="service-icon"><Icon type={i===2?"leaf":i===1?"heart":"star"}/></div><h3>{s.title}</h3><p>{s.desc}</p><Link to={s.to}>{s.label} <span>→</span></Link></div></article>)}</div></div></section>
    ...
    <section className="reviews-section"><div className="container-x"><div className="section-heading"><span className="eyebrow">Real words, real journeys</span><h2>What Our Patients Say</h2></div><div className="reviews-layout"><div className="review-score"><strong>5.0</strong><span>★★★★★</span><small>Rated on Google</small><a href="https://www.google.com/search?q=Ouransh+Mohali+reviews" target="_blank" rel="noreferrer">Read all reviews on Google →</a></div><div className="review-marquee"><div className="review-track">{[...reviews,...reviews].map(([name,text],i)=><article key={name+i}><div className="review-top"><span>★★★★★</span><b>G</b></div><p>“{text}”</p><strong>— {name}</strong></article>)}</div></div></div></div></section>
